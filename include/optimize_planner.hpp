...
class EuclidianDis_optimize_planner
{
public:
     EuclidianDis_optimize_planner()
     {
    ...
    bool start_planning()
    {
        ...
        //sample_si->setStateValidityChecker(boost::bind(&EuclidianDis_optimize_planner::isStateValid,_1));

        ob::ScopedState<> start_state(sample_si);
        ob::ScopedState<> goal_state(sample_si);
...
private:
     ...
     {
         std::cout<<"Joint_bounds dimenstion: "<<Joint_bounds.getVolume()<<std::endl;
         // JointSpace Limits
         if( motoman_arm_DOF)
         {
         ...
         {
            ROS_INFO("Joint Bounds dimension is not as same as Joint Numbers!");
            return false;
         }
     }

     ob::ScopedState<> SetState(std::vector<double>& state_config, ob::SpaceInformationPtr smp_space)
     {
